...
// add S3/etc. here when a board joins. per-board GPIO pin map is the
...
const RUNTIME_FILES = [
  ...
  "core/locale.h",
  "core/lexer.h",
  "runtime-device/hal.h",
  "runtime-device/hal_esp32.cpp",
];
const INO =
  'extern "C" int aksa_main(void);\nvoid setup() { aksa_main(); }\nvoid loop() {}\n';
...
if (!existsSync(AKSA)) {
  console.error("missing ./aksa binary — run `make aksa`");
  process.exit(1);
}
if ((await run(["arduino-cli", "version"])).code !== 0) {
...
const dataDir = (
  await run(["arduino-cli", "config", "get", "directories.data"])
).stdout.trim();
const bootApp0 = (
...
if (!bootApp0) {
  console.error(
    "esp32 core not installed — run `arduino-cli core install esp32:esp32`",
  );
  process.exit(1);
}
...
async function compile(source: string, locale: string, board: string) {
  ...
  try {
    ...
    const emit = await run([AKSA, "emit", src, "--locale", locale], {
      cwd: ROOT,
    });
    ...
    for (const f of RUNTIME_FILES) {
      ...
      const text = (await readFile(join(ROOT, f), "utf8")).replaceAll(
        '#include "locale.h"',
        '#include "ak_locale.h"',
      );
      await writeFile(
        join(sketch, name.startsWith("locale.") ? "ak_" + name : name),
        text,
      );
    }
    await writeFile(join(sketch, "aksa.ino"), INO);
    await writeFile(
      join(sketch, "program.c"),
      "#define main aksa_main\n" + emit.stdout,
    );
    const localeJson = await readFile(
      join(ROOT, "locales", `${locale}.json`),
      "utf8",
    );
    await writeFile(
      join(sketch, "aksa_locale.h"),
    ...
    if (cc.code !== 0) {
      console.error(
        `compile failed (timeout=${cc.timedOut}):\n${cc.stderr || cc.stdout}`,
      );
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (req.method !== "POST" || url.pathname !== "/compile")
      return fail(404, "bad_request");
    if (Number(req.headers.get("content-length")) > MAX_BODY)
      return fail(413, "bad_request");
    ...
    if (typeof source !== "string" || source.length > MAX_BODY)
      return fail(400, "bad_request");
    if (
      !/^[a-z]{2,8}$/.test(locale) ||
      !existsSync(join(ROOT, "locales", `${locale}.json`))
    )
      return fail(400, "bad_request");
...
console.log(
  `aksa compile server on :${PORT} (boards ${Object.keys(FQBN).join(", ")})`,
);
