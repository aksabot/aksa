...
int main(void) {
    ...
    /* hello + arithmetic */
    CHECK_STR(run(&ID, "cetak(\"halo\")", eid, NULL), "halo\n");
    CHECK_STR(eid, "");
    CHECK_STR(run(&ID, "cetak(1 + 2 * 3)", NULL, NULL), "7\n");
    CHECK_STR(run(&ID, "cetak((1 + 2) * 3)", NULL, NULL), "9\n");
    CHECK_STR(run(&ID, "cetak(7 % 3, 7 / 2, -2 + 1)", NULL, NULL), "1 3.5 -1\n");
    CHECK_STR(run(&ID, "cetak(\"umur: \" + 9)", NULL, NULL), "umur: 9\n");

    /* booleans print as localized words */
    CHECK_STR(run(&ID, "cetak(benar, salah)", NULL, NULL), "benar salah\n");
    CHECK_STR(run(&EN, "print(true, false)", NULL, NULL), "true false\n");
    CHECK_STR(run(&ID, "cetak(1 < 2, bukan benar, benar dan salah, salah atau benar)",
                  NULL, NULL), "benar salah salah benar\n");

    /* variables: globals, buat, strings */
    CHECK_STR(run(&ID, "x = 5\nx = x + 1\ncetak(x)", NULL, NULL), "6\n");
    CHECK_STR(run(&ID, "buat nama = \"Aksa\"\ncetak(\"halo \" + nama)", NULL, NULL),
              "halo Aksa\n");

    /* control flow */
    CHECK_STR(run(&ID, "jika (2 > 1) { cetak(\"ya\") } lainnya { cetak(\"bukan\") }",
                  NULL, NULL), "ya\n");
    CHECK_STR(run(&ID, "jika (1 > 2) { cetak(\"ya\") } lainnya { cetak(\"bukan\") }",
                  NULL, NULL), "bukan\n");
    CHECK_STR(run(&ID, "ulangi 3 { cetak(\"x\") }", NULL, NULL), "x\nx\nx\n");
    CHECK_STR(run(&ID, "i = 3\nselama (i > 0) { cetak(i)\ni = i - 1 }", NULL, NULL),
              "3\n2\n1\n");
    CHECK_STR(run(&ID, "i = 0\nselama (benar) { i = i + 1\njika (i == 3) { berhenti }\ncetak(i) }",
                  NULL, NULL), "1\n2\n");
    CHECK_STR(run(&ID, "ulangi 5 { berhenti }\ncetak(\"selesai\")", NULL, NULL), "selesai\n");
    ...
    CHECK_STR(eid, "E102");
    run(&ID, "fungsi f(a) { kembali a }\ncetak(f(1, 2))", eid, &eline);
    CHECK_STR(eid, "E103");
    CHECK(eline == 2);
    run(&ID, "cetak(1 + benar)", eid, NULL);
    CHECK_STR(eid, "E104");
    run(&ID, "jika (1) { cetak(1) }", eid, NULL);
