...
static void handleRun() {
    ...
    if (running) { /* preempt: stop the old program, wait for it to end */
        device_request_stop();
        for (int i = 0; i < 200 && running; i++) delay(10); /* up to ~2 s */
        if (running) { server.send(409, "text/plain", "busy"); return; }
    }

    run_loc = server.arg("locale") == "en" ? &loc_en : &loc_id;
    run_src = strdup(src.c_str());
    if (!run_src) { server.send(500, "text/plain; charset=utf-8", "out of memory"); return; }
    device_out_begin();
    running = true;
    if (xTaskCreate(aksaTask, "aksa", AKSA_STACK, nullptr, 1, nullptr) != pdPASS) {
